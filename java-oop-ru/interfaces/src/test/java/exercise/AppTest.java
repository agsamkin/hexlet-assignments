...
class AppTest {
    ...
    void testReversedSequence() {
        String str = "qwerty";
        ReversedSequence reversedSequence = new ReversedSequence(str);

        String expected = "ytrewq";
        assertThat(reversedSequence.toString()).isEqualTo(expected);

        assertThat(reversedSequence.length()).isEqualTo(6);

        assertThat(reversedSequence.charAt(0)).isEqualTo('y');
        assertThat(reversedSequence.charAt(reversedSequence.length()-1)).isEqualTo('q');

        assertThat(reversedSequence.subSequence(1, 4)).isEqualTo("tre");
    }
    // END
}
